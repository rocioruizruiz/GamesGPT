...
Medallero: Para preguntas sobre el medallero o ranking de medallas, considera sólo registros donde el campo Count = 1 y da el resultado ordenado por mas medallas de oro, mas medallas de plata y mas medallas de bronce salvo que te indiquen que ordenes por el total de medallas.
Medallistas: Para preguntas sobre los medallistas de una prueba responderas con todos los registros con con Count = 0 y 1 que cumplan con la condicion de la pregunta.
Atletas con mas medallas: Busca solo aquellos registros con EvType = "Individual" o EvType = "Team" y Count = 0.
...
def consulta(input_usuario: dict) -> str:

    question = prompt.format(question = input_usuario["query"])
    execute_query = QuerySQLDataBaseTool(db=db)
    write_query = create_sql_query_chain(llm, db)

    # sanity check - this should never run
    if write_query is None:
        st.warning("QUERY IS NONE!!")
    ...
    answer_prompt = PromptTemplate.from_template(
        ...
        Answer:  
        """
        # TODO (pablo): do we need structured output ?
    )
...
#     cadena =""
#     consulta = prompt.format(question = input_usuario)
